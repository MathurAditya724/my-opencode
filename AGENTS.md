...
* **Email pipeline: dumb pipe, agent decides — no identity resolution**: Email pipeline is a dumb pipe — no identity resolution. Plugin does: HMAC verify, dedup by \`message\_id\`, dispatch to agent via fire-and-forget (entity\_key=null). Email→PR disconnect is intentional: the email session is isolated; when the agent creates a PR from it, the PR gets its own entity-keyed session starting from the first \`pull\_request\` webhook. The PR body carries context. \`MAX\_EMAIL\_BODY\_BYTES\` is 512 KB. Cloudflare worker: unconditionally forward via \`message.forward(FORWARD\_TO)\` (try/catch), check \`ALLOWED\_SENDERS\`, parse MIME body, POST HMAC-signed JSON to \`WEBHOOK\_URL\`. \`EMAIL\_WEBHOOK\_SECRET\` via \`wrangler secret put\`; runtime vars not in wrangler.json.
...
* **SQLite schema: entities/links/dispatches — kind CHECK blocks non-PR/issue entities**: SQLite schema: three tables — \`entities\` (entity\_key PK, repo, number, kind CHECK('issue','pull\_request'), session\_id, agent, timestamps); \`links\` (source\_key, target\_key, relation, bidirectional index); \`dispatches\` (id PK, entity\_key nullable, session\_id nullable, trigger\_name, event, delivery\_id, status, outcome always NULL, timestamps). The \`kind\` CHECK blocks non-issue/non-PR entities (commits, branches) — requires migration to add. \`entity\_key\` is null for email/push (fire-and-forget). \`setOutcome()\` exists in \`storage.ts\` but is never called — \`status='succeeded'\` only means \`session.prompt()\` resolved, not that the agent succeeded.
...
* **Hono middleware ordering: \`use('/')\` matches POST routes registered after it**: Hono routing in \`handler.ts\`: \`createApp()\` returns \`Hono\<AppEnv>\`. Routes: \`GET /healthz\` (no auth), \`GET /metrics\` (no auth), \`POST /webhooks/github\` (HMAC \`X-Hub-Signature-256\`), \`POST /webhooks/email\` (HMAC \`X-Email-Signature-256\`). Dep-injection middleware scoped to \`/webhooks/\*\` only. \`app.use('\*')\` wraps all requests in Sentry isolation scope. Handler files export plain \`(c: Context\<AppEnv>) => Promise\<Response>\`. Gotcha: \`use('/')\` matches POST routes registered after it — always scope middleware narrowly.
