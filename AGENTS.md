...
* **Entity-keyed session affinity pipeline with idle cleanup and semaphore-gated follow-ups**: \`pipeline.ts\` manages session affinity: entity key (\`owner/repo#N\` from \`entity.ts\`) maps to a \`SessionEntry\` in an in-memory Map. New event → no session: \`createAndPrompt\` acquires semaphore, creates session, sends prompt. Session busy: event queued (stays \`pending\`). Session idle: \`followUp\` acquires semaphore, sends batched follow-up. Critical invariants: (1) idle timer (10 min default) calls \`cleanup()\` to delete Map entry + \`drainCounter.end()\` + clear abort/batch timers — prevents memory leak and drain hang; (2) all paths (error, timeout, idle) go through \`cleanup()\`; (3) \`fireAndForget\` is the inner async function name (NOT \`dispatchNoAffinity\`) to avoid recursive shadowing with the public method.

<!-- lore:019ddf82-a55d-747f-b14b-cb1efd6e4b8f -->
* **Hono plugin: webhook-to-agent dispatch with dashboard and API**: \`handler.ts\` \`createApp()\` returns \`Hono\<AppEnv>\`. \`app.onError\` captures unhandled route throws to Sentry. \`app.use('\*')\` middleware wraps each request in \`Sentry.withIsolationScope\` + \`Sentry.startSpan({ op: 'http.server' })\`, setting tags: \`http.method\`, \`http.route\`, \`delivery.id\`, \`github.event\`. Span status set from \`c.res.status\`. Deps injected via scoped \`app.use\` middleware into \`c.var\`. Routes: \`GET /healthz\`, \`GET /deliveries\`, \`GET /deliveries/:id\`, \`GET /api/entities\`, \`GET /api/entities/:key\`, \`GET /api/stats\`, \`POST /api/dispatches/:id/retry\`, \`GET /dashboard\` (overview), \`GET /dashboard/entities/:key\`, \`GET /dashboard/deliveries\`, \`POST /dashboard/dispatches/:id/retry\`, \`POST /webhooks/github\`, \`POST /webhooks/email\`. \[\[019ddfd8-298f-79db-b305-68d5c5082d7e]]
...
<!-- lore:019de760-ce70-77b9-8fd8-4ae184583f76 -->
* **dispatchNoAffinity method vs inner async function: name shadowing causes infinite recursion**: In \`pipeline.ts\`, the public method \`dispatchNoAffinity\` and the inner async function that does the work must have different names. If both are named \`dispatchNoAffinity\`, the method body's \`void dispatchNoAffinity(...)\` calls the method recursively instead of the async function — causing a stack overflow on every event without an entity key. Fixed by naming the inner async function \`fireAndForget\`. Always use a distinct internal name for the async impl.
...
<!-- lore:019de760-ce74-7784-8d46-8abac9f29532 -->
* **Pipeline session lifecycle: drainCounter.end() must fire exactly once per session via cleanup()**: In \`pipeline.ts\`, \`drainCounter.start()\` is called once when a new session is created. \`drainCounter.end()\` must be called exactly once — in \`cleanup()\` — regardless of whether the session ended via error, timeout, or idle expiry. If \`end()\` is called on error paths but not the idle/happy path, graceful shutdown's \`drainCounter.wait()\` hangs forever. All session termination paths (error in \`createAndPrompt\`, error in \`followUp\`, idle timer expiry) must go through \`cleanup()\`.
