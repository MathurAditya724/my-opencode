...
* **Single unified github-agent replaces 5 specialized agents**: One agent file \`github-agent.md\` (Opus 4.6, \`mode: primary\`) is COPYed to \`/home/developer/.config/opencode/agents/\` at build time. It receives raw webhook payloads, triages the event, and loads situation-specific skills on demand: \`repo-setup\`, \`resolve-issue\`, \`review-pr\`, \`fix-ci\`, \`respond-to-comment\`, \`apply-fixes\`. Also uses utility skills: \`deslop\`, \`review\`, \`pr\`. Agent name passed to \`runOpencode\` must match filename without \`.md\`.
