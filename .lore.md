...
* **Agents lack cross-session access by design**: Agents lack cross-session access by default: the OpenCode SDK client (session.list/get/messages/prompt) exists but is only used inside the pipeline and not exposed to agent tools. Cross-session behavior (affinity, linking) is handled server-side via dispatches \[\[019e19e2-9c09-7787-b48d-ca2d2685b261]]. To enable agents to inspect or interact with other sessions, you must explicitly expose wrapper tools that pass through \`ctx.client\` (e.g. read/post session messages). Without these tools, agents are strictly isolated to their own session context.
