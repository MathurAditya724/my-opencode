...
export function EntityDetailPage({ entity_key, session_id, events }: EntityDetailProps) {
  ...
  return (
    <Layout title={entity_key} activePage="entities">
      <div class="entity-header">
        <span class="entity-key">{entity_key}</span>
        <span class="entity-kind">{kindLabel}</span>
        {session_id ? (
          <span class="mono text-small text-muted" style="margin-left:8px;">
            session: {session_id.slice(0, 8)}…
          </span>
        ) : null}
        <span style="flex:1;" />
        {opencodeUrl ? (
          <a href={opencodeUrl} target="_blank" rel="noopener" class="btn btn-primary">
            Open in OpenCode
          </a>
        ) : null}
          ...
          {events.map((ev) => {
            ...
            return (
              <li class="timeline-item">
                <div class={`timeline-dot ${statusClass(ev.status)}`} />
